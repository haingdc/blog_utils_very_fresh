...
const Jsx: DataDict = {
  items: [
    {
      match:
        /(?<open><\/?)(?<name>[\w.]+\b)(?<props>(?:(?:\s*\s[\w-]+\b)|(?:\s\s*[\w-]+=["'].+?["'])|(?:\s\s*[\w-]+={[\S\s]+?}))*?)(?<close>\s*\/?>)/g,
      handleGroups: {
        open: "TYPE:oper",
        name: "TYPE:class",
        props: "jsx_props",
...
const JavaScript: DataDict = {
  items: [
    ...
    {
      match:
        /(?<start>return\s*\(\s*)(?<jsx><[\w.]+[\S\s]+?(?:<\/[\w.]+?>|\/>))(?<end>\s*\)\s*}\n*)/g,
      handleGroups: {
        start: "ts",
        jsx: "jsx",
        end: "ts",
...
const toSpan = (str: string, tokenType: string | null) =>
  tokenType ? `<span class="shj-syn-${tokenType}">${str}</span>` : str;
...
export function tokenize(
  ...
  while (i < src.length) {
    ...
    data.items.forEach((data, idx) => {
      ...
      if (actMatchData === null) {
        actMatchData = {
          data,
          match: matchedArr,
          endIndex: matchedArr.index! + matchedArr[0].length,
        };
      }
      ...
      if (
        ...
        (matchedArr.index! == actIndex && data.overraped &&
          data.overraped(actData))
      ) {
        actMatchData = {
          data,
          match: matchedArr,
    ...
    if ("sub" in matchedData.data) {
      ...
      if (!newDict) {
        throw new Error(`sub-name "${matchedData.data.sub}" is not in langs.`);
      }
      ...
      Object.entries(matchedData.match.groups!).forEach(([ky, groupedTxt]) => {
        if (ky in handler) {
          ...
          if (handle.startsWith("TYPE:")) {
            ...
            if (!newDict) {
              throw new Error(`sub-name "${handler[ky]}" is not in langs.`);
            }
